...
export async function renderSetup(
  ...
  container.querySelector<HTMLInputElement>(focusMap[startTab])?.focus();
}
...
function bindLoginTab(container: HTMLElement): void {
  ...
  const checkLockout = (): boolean => {
    ...
    if (remaining > 0) {
      errorEl.innerHTML = `<div class="error-box">Too many attempts. Try again in ${remaining}s.</div>`;
      btn.disabled = true;
      setTimeout(() => { if (!checkLockout()) { errorEl.innerHTML = ''; btn.disabled = false; } }, 1000);
      return true;
    }
    return false;
  };
